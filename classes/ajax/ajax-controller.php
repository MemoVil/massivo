...
class AjaxWorker {	
	...
	{		
		if (!Validate::isInt($combination) 
			...
			ddd($this->verifyReference($reference['param']));	
		...
		$sql = sprintf("UPDATE %s set %s = '%s' where id_product_attribute = '%s' LIMIT 1", DATA::COMBINATION, $reference['type'], $reference['param'], $combination);
